...
- **Evidence-based evolution foundation**: `packages/harness/deerflow/skill_evolution/` defines versioned, bounded contracts for trajectory snapshots, successful-trajectory evidence, event clusters, staged skill proposals, and evaluations. Its store contract has process-local and SQLAlchemy implementations with per-user isolation, event idempotency by `(user_id, run_id, extractor_version)`, ready-cluster threshold queries, and compare-and-swap proposal transitions. ORM rows live under `persistence/skill_evolution/`; Alembic revision `0012_skill_evolution` creates the four durable tables. When `skill_evolution.enabled=true`, the owning run worker builds one `skill_evolution.trace` / `evolution` event after journal, workspace-change, delivery-receipt, and terminal-status persistence. The snapshot retains the first visible task input plus the event tail, pairs tool calls/results, records slash/read Skill identity by path and content hash (never the loaded `SKILL.md` body), and redacts request/active Skill secrets plus host user-data/Skill paths. The write is run-scoped and idempotent through `RunEventStore.put_if_absent`; every extraction/storage failure is non-fatal to the user run.
  `skill_evolution/verifier.py` consumes a snapshot plus bounded trusted signals through the pluggable `OutcomeVerifier` protocol. Terminal failure, recognized tests, explicit command expectations, prevalidated artifact existence/schema/value results, environment rewards, and explicit user acceptance produce priority-aware `success` / `failure` / `unknown` evidence with per-check confidence and provenance; equal-priority conflicts become `unknown`, lower-priority conflicts reduce confidence, and optional LLM judgment is supporting-only. Plugin exceptions become bounded unknown evidence without exposing exception text.
  `skill_evolution/eligibility.py` consumes verified outcomes and `skill_evolution.evidence` settings. Admission requires successful evidence at `min_success_confidence` plus at least one enabled complexity signal: tool-count threshold, recovered error, user correction, trusted non-trivial workflow, or explicit remember/create-skill request. It selects `no_skill` vs. `skill_used` and rejects safety-, loop-, token-, and subagent-limit-capped runs.
  `skill_evolution/extractor.py` performs deterministic pre-extraction only for eligible traces. Its hash-validated intermediate contract contains the event branch, environment/outcome/complexity, observed Skill identities, artifact paths, ordered content-free tool steps (argument/result hashes), and at most 64 semantic candidate segments. Error steps and the execution tail are retained preferentially; large text is head/tail truncated, while probable binary, data-URI, encoded, or control-byte payloads in tool arguments/results are omitted with only a SHA-256 reference. No LLM is called in this phase.
  `skill_evolution/structured_extractor.py` performs provider-neutral structured LLM extraction over the bounded candidates. It embeds a strict Pydantic JSON schema, rejects prose/Markdown output, validates every cited segment, enforces new-skill/patch target rules, and retries only malformed or recognized transient failures within a bounded budget. Valid output becomes a deterministic-ID `EvolutionEvent` with model/Prompt/pre-extraction/snapshot versions and per-semantic-item evidence links. `extract_and_persist()` reaches `SkillEvolutionStore.upsert_event()` only after all validation succeeds. `skill_evolution.extraction_model_name` selects the model (null means primary).
  `skill_evolution/grouping.py` builds hash-validated event fingerprints from normalized task signature/goal tokens, ordered tools, error categories, environment family, event kind, and target Skill. Candidate similarity is deterministic and requires both the configured weighted threshold and a workflow anchor. User, new-Skill/patch branch, and patch-target boundaries are hard partitions; repeated event IDs, runs, task-input hashes, and identical fingerprints are deduplicated before clustering. Conflicting payloads for one event ID are rejected. Output is input-order-stable, bounded, and remains in `collecting` state with per-member `GroupingEvidence`; it never marks `K=3` readiness. `skill_evolution.grouping.deterministic_threshold` defaults to `0.65`.
  `skill_evolution/semantic_retrieval.py` optionally broadens deterministic candidates through provider-neutral `EmbeddingProvider` and `SemanticVectorStore` protocols. The built-in local baseline uses Ollama `/api/embed` plus Qdrant REST through the core `httpx` dependency. Structured event documents exclude event/user identity and are bounded before embedding. Physical collections are model/version-isolated; point payloads retain immutable model digest, document hash, user, event kind, and patch target, and queries reapply those hard partitions. Returned candidates carry deterministic/semantic scores and source provenance. Embedding is disabled by default, and any embedding/Qdrant failure returns deterministic candidates without exposing service error text. Config lives under `skill_evolution.grouping.{semantic_threshold,semantic_top_k,embedding,vector_store}`.
  `skill_evolution/cluster_confirmation.py` compares every non-prototype candidate with the prototype through a strict JSON-schema LLM decision. Candidate text is bounded untrusted data; malformed/transient output has bounded retry and then remains unconfirmed. Membership requires pre-existing deterministic or semantic provenance plus LLM agreement. The model distinguishes same workflow, compatible environment-specific branches, and different workflows; accepted branch conditions and contradictions are persisted per member together with unique deterministic/semantic/LLM evidence and confirmation model/Prompt metadata. Readiness recomputes unique `run_id` values and requires `min_cluster_events` plus `min_distinct_runs` (both default 3), no contradictions, and no unconfirmed candidates. The final immutable snapshot can be stored through `put_cluster()` and queried by `list_ready_clusters()`.
  `skill_evolution/distiller.py` converts only ready new-Skill clusters into deterministic-ID `operation=create`, `status=staged` Proposals. Its strict JSON-schema prompt contains structured events plus bounded provenance excerpts and separates repeated common/verification rules, environment conditions, conflicts, unsupported one-offs, and optional support files. Mandatory guidance, conflicts, and files require at least two distinct source runs; unknown/single-run evidence retries then fails closed. Deterministic rendering owns complete `SKILL.md` structure, while model output supplies typed content. Every file/semantic section has event mappings; Proposals retain source Cluster hash and distiller model/Prompt. Conflicts and executable files force manual review, one-offs stay out of instructions, and risk aggregation is bounded. `distill_and_persist()` only calls `put_proposal()` and never writes/publishes a Skill. `skill_evolution.distillation_model_name` selects the model; it was introduced in config schema version 39.
  `skill_evolution/patch_distiller.py` converts only ready existing-Skill clusters into targeted `operation=patch`, `status=staged` Proposals. `SkillStorageVersionSource` resolves the exact trajectory-observed hash from current content or JSONL `prev_content`/`new_content`, while every generated operation targets the latest content. Output is restricted to exact `SKILL.md` `find`/`replace` with `expected_count=1`; full-file replacement, ambiguous/missing targets, invalid frontmatter/name changes, mixed source hashes, and unsupported evidence fail closed. General changes require two distinct runs; one-environment changes require an explicit condition. Operations apply only in memory and preserve unaffected content. A post-model hash read detects concurrent edits and restarts against the latest base. The staged Proposal stores both source hash provenance and current `base_skill_hash` for future publication CAS, plus structured operations and per-operation mappings. No Skill write occurs.
  `skill_evolution/evaluator.py` defines versioned replay tasks, disposable evaluation workspaces, and paired new-Skill evaluation. A replay task binds one successful snapshot/event pair to its task family; a bounded, binary-safe, hash-validated fixture; OS/runtime/command/secret/network requirements; deterministic command/artifact verifiers; timeout; and a side-effect policy. Automatic replay is fail-closed: missing task input/verifiers, truncated traces, incomplete fixtures, external credentials, or blocked required network produce explicit manual-review reasons. Fixture capture rejects traversal and symlinks and runs off-loop. `isolated_replay_workspace()` copies fixtures only into temporary `workspace/`, exposes `outputs/`, materializes staged/validating same-user candidate files read-only under temporary `skills/`, audits changed files/bytes against policy, and removes the tree after normal or exceptional exit; cleanup must skip chmod on symlinks so it never follows an external target.
  `NewSkillProposalEvaluator` accepts only create Proposals and requires source tasks to exactly cover supporting events; source and held-out tasks are unique, independent, same-user, and same-family. Every task runs in a separate no-Skill workspace and candidate-Skill workspace. Agent and command execution go through the injected `ReplayRuntime`, which owns actual process/sandbox isolation and must force candidate activation from the explicit path; the evaluator never invokes a host shell. It applies whole-agent/per-command timeouts, deterministic command/artifact checks, side-effect policy, and records tool/token/latency metrics, stable error codes, and artifact kind/hash/size without content. Source and held-out thresholds come from `skill_evolution.quality` (defaults `1.0`/`0.8`; config schema version 40). `evaluate_and_persist()` is evaluation-ID idempotent, transitions staged to validating, persists raw results, and transitions failed candidates to rejected; passed/manual-review candidates remain validating. Aggregate `quality_score` is explicitly deferred to Task 6.4.
  The worker does not invoke verification, eligibility, pre-extraction, structured extraction, grouping, semantic retrieval, cluster confirmation, either distiller, replay preparation, or proposal evaluation yet. No production coordinator/runtime adapter builds replay suites automatically. Existing-Skill regression evaluation, aggregate quality, approval, and publication remain unimplemented, so the current hook never modifies a Skill. Continue implementation from `docs/plans/2026-08-16-self-evolving-skills-development-plan.md`.
