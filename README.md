...
With `skill_evolution.enabled: true`, completed runs also retain one bounded, redacted `skill_evolution.trace` event for evidence-based Skill improvement. The snapshot records tool outcomes, Skill path/hash usage, corrections, and artifacts while removing request-scoped secrets, host paths, hidden framework context, and loaded `SKILL.md` bodies. The backend can conservatively verify these snapshots from deterministic tests, command expectations, prevalidated artifact checks, environment rewards, or explicit user acceptance; model self-reported success is never authoritative. Its eligibility gate then requires high-confidence success plus a configured complexity signal and rejects safety/resource-capped runs. Eligible traces can be deterministically reduced to hash-addressed tool structure and bounded semantic excerpts without embedding binary/encoded payloads, then explicitly passed through strict JSON-schema LLM extraction to create a provenance-linked `EvolutionEvent`. Persisted events can be deterministically fingerprinted and grouped into user- and branch-isolated candidate clusters while deduplicating repeated runs. Optional Ollama embeddings and Qdrant retrieval can broaden that candidate set while retaining model/version and score provenance; either service may be disabled or unavailable, in which case retrieval remains deterministic. Strict structured LLM confirmation then preserves compatible environment-specific branches, excludes different workflows, and records contradictions. A cluster becomes `ready` only with at least three confirmed events from three distinct runs and no contradiction or unconfirmed candidate; repeated evidence from one run cannot satisfy the threshold. Ready new-Skill clusters can be distilled into complete, evidence-mapped `SKILL.md` packages held in `staged` Proposals. Ready existing-Skill clusters resolve the exact historical version used by source runs, generate targeted evidence-linked patch operations against the current version, and restart if that base changes during generation. Mandatory guidance and support files require repeated cross-run evidence; conflicts and executable files force manual review, and unsupported one-off observations are excluded from instructions. Automatic processing, evaluation, approval, and publication are not wired into a background processor yet, so this does not automatically update or publish a Skill.

Successful source evidence can also be converted into a versioned replay task that binds its task family, bounded binary-safe fixtures, environment requirements, deterministic command/artifact verifiers, timeout, and an allowed-side-effect policy. Tasks that depend on missing inputs, truncated traces, external secrets, or blocked network access are marked manual-review-only. Replay inputs and staged candidate Skills can be materialized in a disposable workspace with read-only candidate files, automatic cleanup, and side-effect auditing, without modifying the source workspace or production Skill library. Given an isolated `ReplayRuntime`, the new-Skill evaluator runs every source and same-family held-out task as a paired no-Skill/candidate experiment, records success, tool calls, tokens, latency, redacted errors, and artifact hashes, and persists an idempotent evaluation. Candidates below the configured source or held-out pass threshold are rejected even if they fit their source traces. Aggregate quality, existing-Skill regression evaluation, approval, and publication remain separate later phases, and the runtime worker does not invoke this pipeline automatically.
