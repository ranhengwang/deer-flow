...
The Skill evolution pipeline emits content-free structured lifecycle logs for admission, rejection, extraction, clustering, readiness, distillation, evaluation, approval, publication, and rollback. User identity and Skill name are hashed; events otherwise contain only bounded IDs, hashes, reason codes, decisions, counts, and timestamps. Process-local low-cardinality metrics track durable queue depth, active jobs, extraction failures, Cluster purity, proposal pass rate, publication rate, regression rate, and quality lift. The snapshot is available to Gateway integrations through `app.state.skill_evolution_observability`; no public `/metrics` endpoint is exposed.

Authenticated users can inspect their persisted evolution history through `GET /api/skill-evolution/{events,clusters,proposals,evaluations,versions}`. Every collection is bounded by `limit`/`offset`, requires `skill_evolution:read`, and returns compact records without trajectory prose, Proposal file contents, replay errors, safety payloads, or package snapshots. The default scope is the current owner; only an admin may supply another `user_id`, and the admin still needs the route permission.

Explicit human review and version mutations are available through `POST /api/skill-evolution/proposals/{proposal_id}/review`, `POST /api/skill-evolution/proposals/{proposal_id}/publish`, and `POST /api/skill-evolution/versions/{publication_id}/rollback`. They require separate `skill_evolution:review`, `skill_evolution:publish`, and `skill_evolution:rollback` permissions plus CSRF validation. The authenticated identity supplies the reviewer/rollback actor and current time; clients cannot override either. Operations default to the caller's owner scope, while cross-user administration requires both admin status and the route permission. These routes remain the workflow for `publication.mode: manual`; `publication.mode: direct` bypasses Evaluation and review and is performed only by the background pipeline.

For future RL experiments, DeerFlow now persists structured selection, utilization, and distillation Credit records. Selection records capture observed Skill search/activation decisions and a deterministic utility trend; unavailable rank scores or provider log probabilities remain explicitly null. Utilization records bind verified task rewards and content-free tool evidence to the exact Skill hash. Published versions retain source baselines and raw future outcome series, producing distillation marginal utility only after three independent future runs use that exact version. Credit logging is fail-open and never changes an online task, publication, or rollback result.

The Phase 11 experiment harness freezes 60 deterministic-verifier task slots across repository repair, structured data, and shell workflows, plus 12 baseline/ablation conditions and three-seed pairing. `backend/scripts/benchmark/skill_evolution/run.py` validates or executes the matrix through an external Replay executor; `--resume` restores only complete family cohorts and atomically checkpoints each newly completed cohort. `summarize.py` reports aggregate and per-family metrics, paired bootstrap intervals, exact McNemar tests, and failed/rejected counts. The bundled smoke executor is synthetic and schema-labelled `deterministic_smoke`; it validates plumbing only and must not be cited as model performance. The production executor materializes real fixtures, uses Qwen-generated create/Patch candidates, and emits `production_replay` rows with immutable model/version/prompt/environment identity.

With `skill_evolution.enabled: true`, completed runs also retain one bounded, redacted `skill_evolution.trace` event for evidence-based Skill improvement. The snapshot records tool outcomes, Skill path/hash usage, corrections, and artifacts while removing request-scoped secrets, host paths, hidden framework context, and loaded `SKILL.md` bodies. The backend can conservatively verify these snapshots from deterministic tests, command expectations, prevalidated artifact checks, environment rewards, or explicit user acceptance; model self-reported success is never authoritative. Its eligibility gate then requires high-confidence success plus a configured complexity signal and rejects safety/resource-capped runs. Eligible traces can be deterministically reduced to hash-addressed tool structure and bounded semantic excerpts without embedding binary/encoded payloads, then passed through strict JSON-schema LLM extraction to create a provenance-linked `EvolutionEvent`. The deterministic create/Patch branch is authoritative: create extraction discards model-emitted Patch-only target/gap fields, and unsupported optional failed-attempt or correction claims are omitted instead of invalidating otherwise grounded required evidence. Persisted events can be deterministically fingerprinted and grouped into user- and branch-isolated candidate clusters while deduplicating repeated runs. Optional Ollama embeddings and Qdrant retrieval can broaden that candidate set while retaining model/version and score provenance; either service may be disabled or unavailable, in which case retrieval remains deterministic. Strict structured LLM confirmation then preserves compatible environment-specific branches, excludes different workflows, and records contradictions. A cluster becomes `ready` only with at least three confirmed events from three distinct runs and no contradiction or unconfirmed candidate; repeated evidence from one run cannot satisfy the threshold. Ready new-Skill clusters can be distilled into complete, evidence-mapped `SKILL.md` packages held in `staged` Proposals. Ready existing-Skill clusters resolve the exact historical version used by source runs, generate targeted evidence-linked patch operations against the current version, and restart if that base changes during generation. Mandatory guidance and retained support files require repeated cross-run evidence; unsafe absolute, traversal, or reserved optional support paths emitted by a model are discarded before full schema validation, while valid files still pass evidence and publication security checks. Unsupported one-off observations are excluded from instructions. A durable background coordinator runs verification through Proposal distillation outside the request path. SQL jobs use idempotency keys, renewable leases, bounded concurrency, retry backoff, startup recovery, and bounded shutdown drain; its in-memory queue is only a wake-up hint. In the default `manual` mode the online coordinator stops at `staged`. The explicit high-risk `direct` mode immediately publishes that staged Proposal without Replay Evaluation or approval, while still enforcing package/path validation, SkillScan, LLM security moderation, base/package hash CAS, atomic replacement, version snapshots, history, and rollback. Direct mode also removes `skill_manage` and its active self-evolution instructions from the lead agent, so only the K-ready background pipeline can perform automatic Skill writes.

Successful source evidence can also be converted into a versioned replay task that binds its task family, bounded binary-safe fixtures, environment requirements, deterministic command/artifact verifiers, timeout, and an allowed-side-effect policy. Tasks that depend on missing inputs, truncated traces, external secrets, or blocked network access are marked manual-review-only. Replay inputs and staged candidate Skills can be materialized in a disposable workspace with read-only candidate files, automatic cleanup, and side-effect auditing, without modifying the source workspace or production Skill library. `DockerReplayRuntime` additionally runs disposable containers with no network, read-only root filesystems, dropped capabilities, no-new-privileges, bounded CPU/memory/PIDs, writable workspace/output mounts, and read-only Skill mounts; production benchmark tasks expose only `read_file` and structured `write_json` to the model. The new-Skill evaluator runs every source and same-family held-out task as a paired no-Skill/candidate experiment. The Patch evaluator requires a complete base Skill package bound to the Proposal hash, overlays candidate files in memory, and runs paired old/new source and historical regression tasks. Both persist success, tool calls, tokens, latency, redacted errors, and artifact hashes. New Skills below source or held-out thresholds and patches above the configured old-success/new-failure regression rate are rejected; executable supporting-file changes require manual review. Deterministic `skill-quality-v1` scoring records success lift, held-out generalization, efficiency, environment robustness, regression resistance, and safety separately from raw results. A high-quality designation requires at least five candidate tasks, two held-out or regression samples, two environments, an approved evaluation, no safety blocker, and an aggregate score of at least `0.75`; three source runs alone cannot qualify.

`skill-approval-v1` then applies a deterministic approval policy for `manual` and `eligible_auto` workflows. `skill_evolution.publication.mode` defaults to `manual`; `eligible_auto` can only mark a non-executable, low-risk Patch `approved` after an approved evaluation with fully successful candidate held-out results. `direct` deliberately skips Evaluation and approval, including distiller review flags, because later offline experiments own effectiveness measurement; it must therefore be treated as a high-risk operator opt-in. Proposals expire after 180 days, and every status change records its source, reason, evaluation when present, reviewer where applicable, policy version, and time.

Agent-managed custom-Skill changes pass through one reusable mutation service. It preserves path and content validation, per-user/per-Skill serialization, native SkillScan, LLM moderation, history, sandbox projection refresh, and prompt-cache refresh for both the existing `skill_manage` tool and Evolution publication. Complete-package publication additionally checks the current `SKILL.md` and whole-package hashes under the final storage lock, then exchanges the staged directory as one mutation boundary.

An approved Proposal can be explicitly published with a binary-safe snapshot of every package file, its executable state, and its base/candidate/published hashes. In `direct` mode the background coordinator invokes the same publisher from `staged`, with a null `evaluation_id` that explicitly records the skipped gate. Both paths persist the snapshot before the Skill write. Rollback reconstructs the exact base package from that record rather than from model output, reruns full SkillScan plus executable moderation, restores the package atomically, and records a new history entry; rolling back a newly created Skill deletes it.
